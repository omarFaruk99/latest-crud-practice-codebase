...
import callToast, {formattedDateTime, handleDelete, tablePerPageOptions} from "@/app/utilis/helper";
...
const IngredientTestThree = () => {
    // ==========>state

    // ========== Context ==========
    const { accessToken } = useContext(LayoutContext);
    // table data
    const [foodIngredientData, setFoodIngredientData] = useState<InventoryTrace[]>([])
    ...
    const [ingredientForm, setIngredientForm] = useState<any>({
    ...
    // ================>data fetching
    // main table data
    const {data: foodItems, loading, totalData, notFound} = useFetch(endpoint,[update])
    ...
    useEffect(() => {
        if (foodItems) {
            setFoodIngredientData(foodItems);
        }
    }, [foodItems, notFound]);
    ...
    const handleCreate = async (id?: number | null) => {
        if (!ingredientForm.food_ingredient_id?.id) {
            return callToast(toast, false, "food ingredient is required");
        }
        if (!ingredientForm.supplier?.id) {
            return callToast(toast, false, "supplier is required");
        }
        if (!ingredientForm.quantity) {
            return callToast(toast, false, "quantity is required");
        }
        if (!ingredientForm.total_amount) {
            return callToast(toast, false, "total amount is required");
        }

        const payload = {
            food_ingredient_id: ingredientForm.food_ingredient_id.id,
            supplier_id: ingredientForm.supplier?.id,
            quantity: ingredientForm.quantity,
            total_amount: ingredientForm.total_amount,
        };
    ...
    return (
        <div className={"card"}>
            <Toast ref={toast}/>
            <ConfirmPopup/>
            <PageHeading title={"Test Ingredient Three"} isCreate={true}
            ...
            >
                <div className="field">
                    ...
                    <Dropdown
                        value={ingredientForm.food_ingredient_id}
                ...
                <div className="field">
                    ...
                    <InputNumber
                        id="total_amount"
                        value={ingredientForm.total_amount}
...
export default IngredientTestThree;
